...
def telegram_webapp_auth(
    ...
    if not isinstance(init_data, str) or not init_data.strip():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="initData required"
        )

    try:
        parsed = verify_telegram_webapp_init_data(
            init_data=init_data, settings=settings
        )
    ...
    if not isinstance(user, dict) or not isinstance(user.get("id"), int):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Missing Telegram user"
        )
    ...
    try:
        token = encode_access_token(
            payload=AccessTokenPayload(
                user_id=str(db_user.id), telegram_id=db_user.telegram_id, exp=exp
            ),
            settings=settings,
        )
