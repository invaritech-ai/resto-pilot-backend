...
### K) Capability lock-down (one capability at a time)
- [x] Add `enabled_capabilities_csv` to `Settings` in `app/core/config.py`
- [x] Add `classify_capability()` in `app/ai/capability_gate.py` (hard reject unsupported work)
- [x] Modify `process_session()` in `app/processing/session_processor.py` to enforce capability lock and ghost after first reject
- [x] Modify `generate_session_reply_with_metrics()` in `app/ai/session_reply.py` to run without tools + enforce employee-style output guards
