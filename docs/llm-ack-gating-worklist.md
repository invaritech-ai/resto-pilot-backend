...
- [x] Review autogenerated migration `upgrade()` (tables/columns/indexes/FKs are correct)
- [x] Review autogenerated migration `downgrade()` (drops in safe order)
...
- [x] Add `estimate_cost_usd()` in `app/ai/openrouter_cost.py` (computes cost from usage + env-configured price map)
...
- [x] Add `record_outgoing_message()` in `app/workers/telemetry.py` (inserts into `telegram_outgoing_messages`)
- [x] Add `get_or_create_chat_state()` in `app/workers/telemetry.py` (ensures a `telegram_chat_states` row)
- [x] Add `set_chat_off_topic()` in `app/workers/telemetry.py` (sets off-topic mode + timestamp)
- [x] Add `set_chat_on_topic()` in `app/workers/telemetry.py` (clears off-topic mode + timestamp)
...
### J) Contextual Memory (summary + last 50 messages)
- [x] Add model `TelegramChatMemory` in `app/db/models/telegram_chat_memory.py`
- [x] Add `load_chat_memory_summary()` in `app/ai/chat_memory.py`
- [x] Add `load_chat_history_messages()` in `app/ai/chat_memory.py` (last 50, includes bot + user)
- [x] Add `update_chat_memory_summary()` in `app/ai/chat_memory.py` (cheap model summarizer)
- [x] Modify `generate_session_reply_with_metrics()` in `app/ai/session_reply.py` to accept + use memory context
- [x] Modify `process_session()` in `app/processing/session_processor.py` to pass memory context + update summary after replies
- [x] Review autogenerated migration for `telegram_chat_memory` (after autogenerate)

### K) Capability lock-down (one capability at a time) [DEPRECATED]
- [x] Add `enabled_capabilities_csv` to `Settings` in `app/core/config.py` [REMOVED]
- [x] Add `classify_capability()` in `app/ai/capability_gate.py` (hard reject unsupported work) [REMOVED]
- [x] Modify `process_session()` in `app/processing/session_processor.py` to enforce capability lock and ghost after first reject [REPLACED]
- [x] Modify `generate_session_reply_with_metrics()` in `app/ai/session_reply.py` to run without tools + enforce employee-style output guards [REMOVED]

**Replaced by**: General-purpose agent with tool-calling (`app/ai/agent.py`) and role/scope-based access control (`app/policies/db_policy.py`). See `docs/capabilities.md` for migration notes.
