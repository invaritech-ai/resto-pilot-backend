...
- [ ] Add model `LLMCalls` in `app/db/models/llm_calls.py`
- [ ] Add model `TelegramOutgoingMessages` in `app/db/models/telegram_outgoing_messages.py`
- [ ] Add model `TelegramChatStates` in `app/db/models/telegram_chat_states.py`
...
- [ ] Add `chat_completions_create_with_http_info()` in `app/ai/openai_client.py` (returns `(data, headers, latency_ms)`)
