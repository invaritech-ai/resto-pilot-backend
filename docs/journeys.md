...
The current bot architecture is **default-deny** and runs in a strict “employee mode”.
In practice:
- The bot must not claim access to data or actions that don’t exist yet.
- Capabilities are enabled explicitly and enforced before/after the LLM.
- If a request is outside the enabled capability, the user gets **one** reject message and then the bot **ghosts** until the user returns to an allowed request.

Reference: `docs/capabilities.md`
...
- **Capability implications**: what capability(ies) likely need to exist (or be enabled) to support it end-to-end
