...
Note: The extraction functions do NOT require a database connection.
They read schema info from SQLAlchemy model metadata, not from the database.
"""
...
def test_price_list_extraction(pdf_path: str) -> None:
    ...
    print(
        f"[CONFIG] vision_api_key: {'***' + settings.vision_api_key[-4:] if settings.vision_api_key else 'NOT SET'}"
    )
    ...
    try:
        ...
        print(
            f"[EXTRACT] This is the exact function used by process_price_list_file_task"
        )
        print(
            f"[EXTRACT] Note: No database connection needed - schema read from model metadata"
        )
        print(
            f"[EXTRACT] Starting extraction (this may take a while for large PDFs)..."
        )
...
def test_invoice_extraction(file_path: str) -> None:
    ...
    try:
        ...
        print(f"\n[RESULT] --- Extracted Data ---")
        print(
            json.dumps(extracted_data, indent=2, ensure_ascii=False, default=str)[:3000]
        )
